      ...
      <div className="mx-auto max-w-7xl px-6 lg:px-8">
        <FadeIn>
          <div className="mx-auto max-w-2xl lg:text-center">
            ...
            <p className="mt-2 text-3xl font-bold tracking-tight text-[#36454F] sm:text-4xl font-serif">A Glimpse Into Our Gallery</p>
            <p className="mt-6 text-lg leading-8 text-gray-700">Every photo tells a story. Explore our collection of moments captured for our wonderful clients. Click on any image to view it in full detail.</p>
          </div>
        </FadeIn>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
            {portfolioImages.map((photo, idx) => (
              <FadeIn key={photo.src + idx}>
                <div className="group cursor-pointer" onClick={() => setIndex(idx)}>
                  <div className="overflow-hidden rounded-lg">
                    <img src={photo.src} alt={`Portfolio image ${idx + 1}`} className="h-full w-full object-cover object-center transition-transform duration-300 group-hover:scale-105" />
                  </div>
                </div>
              </FadeIn>
            ))}
        ...
        <Lightbox open={index >= 0} index={index} close={() => setIndex(-1)} slides={portfolioImages} />
      </div>
    </div>
  );
}